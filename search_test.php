...
if (isset($_GET['search'])) {
	...
	$my_search_queries = synonym_queries($_GET['search'], $stop_words_array);
 ...
 *  * */
 		foreach (explode(" ", $my_search_query) as $sword){
			 ...
			 echo "</br>word = ". $sword . "</br>";

			$mysqli = connect ();
			//you are free to put what you want as parameter in this query
			$q = 'SELECT prod_id FROM allproducts WHERE prod_name LIKE \'%'.$sword.'%\' AND prod_description LIKE \'%'.$sword.'%\'';
			$result = $mysqli->query ($q);
		
			$table  = mysqli_fetch_array($result);
			if(!isset($table['prod_id'])){continue;}
			
			if(!isset($results_array[$table['prod_id']])){$results_array[$table['prod_id']] = 0; }
			$results_array [$table['prod_id']] = intval($results_array[$table['prod_id']]) + 1;
			//
...
// now we can display them
			foreach ($sorted_results as $relevant_result) {
				...
				$result = $mysqli->query('SELECT * FROM allproducts WHERE prod_id = '.$relevant_result);
				// echo $result;
				$table  = mysqli_fetch_array($result);
				echo '<h2><a href="product.php?prod_id='.$table['prod_id'].'">'.$table['prod_name'].'</a></h2>';
				...
				echo '<p>'.$table['prod_description'].'</p>';
